...
SEEN_DOPPLER_TOKENS_FILE = "seen_doppler_tokens.json"
...
class ClankerMonitor(commands.Cog):
    def __init__(self, bot):
        self.bot = bot
        self.seen_tokens: Set[str] = self._load_seen_tokens()
        self.seen_doppler_tokens: Set[str] = self._load_seen_doppler_tokens()
        self.channel = None
        self.is_active = True
        self.doppler_active = True
        self.premium_only = False
        self.bankr_enabled = True
        self.img_required = False
        self.tracked_clanker_tokens = {}
        self.tracked_doppler_tokens = {}
        ...
        # --- Ajout du contrat factory Doppler ---
        self.doppler_factory = self.w3_ws.eth.contract(
    ...
    def _load_seen_doppler_tokens(self) -> Set[str]:
        """Load previously seen Doppler token addresses from file."""
        try:
            if os.path.exists(SEEN_DOPPLER_TOKENS_FILE):
                with open(SEEN_DOPPLER_TOKENS_FILE, 'r') as f:
                    return set(json.load(f))
            return set()
        except Exception as e:
            logger.error(f"Error loading seen Doppler tokens: {e}")
            return set()

    def _save_seen_doppler_tokens(self):
        """Save seen Doppler token addresses to file."""
        try:
            with open(SEEN_DOPPLER_TOKENS_FILE, 'w') as f:
                json.dump(list(self.seen_doppler_tokens), f)
        except Exception as e:
            logger.error(f"Error saving seen Doppler tokens: {e}")
    ...
    @commands.command()
    async def doppleron(self, ctx):
        """Activer le monitoring pour Doppler"""
        self.doppler_active = True
        await ctx.send("✅ Monitoring Doppler activé")

    @commands.command()
    async def doppleroff(self, ctx):
        """Désactiver le monitoring pour Doppler"""
        self.doppler_active = False
        await ctx.send("❌ Monitoring Doppler désactivé")

    @commands.command()
    async def lastdoppler(self, ctx):
        """Fetch and display the latest token from Doppler factory"""
        try:
            # Send initial message
            status_msg = await ctx.send("🔍 Recherche du dernier token Doppler...")
            
            # Get the latest block to find recent Doppler deployments
            latest_block = self.w3_ws.eth.block_number
            
            # Search for recent create function calls from Doppler factory
            # Since Doppler doesn't have a TokenCreated event, we'll look for create function calls
            # We'll need to decode the transaction input data manually
            
            # Get recent transactions to the Doppler factory
            block = self.w3_ws.eth.get_block(latest_block, full_transactions=True)
            doppler_txs = []
            
            # Look through recent blocks for Doppler factory transactions
            for block_num in range(latest_block - 100, latest_block + 1):
                try:
                    block = self.w3_ws.eth.get_block(block_num, full_transactions=True)
            ...
            if not doppler_txs:
                await status_msg.edit(content="❌ Aucun token Doppler récent trouvé dans les derniers blocs.")
                return
            ...
            try:
                # Decode the create function call
                func_obj, func_args = self.doppler_factory.decode_function_input(latest_tx['input'])
                
                # Extract token information from the create function
                initial_supply = func_args['initialSupply']
                recipient = func_args['recipient']
                owner = func_args['owner']
                salt = func_args['salt']
                data = func_args['data']
                
                # Try to extract token name and symbol from data
                name = "Unknown"
                symbol = "UNK"
                try:
                    # The data might contain encoded token information
                    # This is a simplified approach - you might need to adjust based on actual data format
                    if data:
                        # Try to decode as string if possible
                        try:
                            decoded_data = data.decode('utf-8').strip('\x00')
                            if decoded_data:
                                name = decoded_data[:20]  # First 20 chars as name
                                symbol = decoded_data[:5].upper()  # First 5 chars as symbol
                        except:
                            pass
                except Exception:
                    pass
                ...
                embed.add_field(name="Supply Initial", value=f"{initial_supply:,}", inline=True)
                ...
                # Add Definitive chart button
                definitive_url = f"https://app.definitive.fi/{recipient}/base"
                chart_button = discord.ui.Button(
                    style=discord.ButtonStyle.primary,
                    label="Voir la Chart",
                    url=definitive_url
                ...
                await status_msg.delete()
                await ctx.send(embed=embed, view=view)
                logger.info(f"Last Doppler command executed for {name} ({symbol}) {recipient}")
                
            except Exception as e:
                logger.error(f"Error decoding Doppler transaction data: {e}")
                await status_msg.edit(content="❌ Erreur lors du décodage des données de la transaction Doppler.")
                
        except Exception as e:
            logger.error(f"Error fetching latest Doppler token: {e}")
            if status_msg:
                await status_msg.edit(content="❌ Erreur lors de la recherche du dernier token Doppler.")
            else:
                await ctx.send("❌ Erreur lors de la recherche du dernier token Doppler.")
    ...
    async def listen_onchain_clanker(self):
        ...
        # Railway-specific: Add connection retry logic
        max_retries = 5
        retry_delay = 10

        while True:  # Boucle principale de reconnexion
            try:
                # Railway-specific: Check WebSocket connection health
                if not self.w3_ws.is_connected():
                ...
                while True:  # Boucle de lecture des événements
                    try:
                        # Railway-specific: Check connection health periodically
                        if not self.w3_ws.is_connected():
                            logger.warning("WebSocket connection lost during event loop, breaking to reconnect...")
                            break
                        ...
                        # Railway-specific: Longer sleep to reduce resource usage
                ...
                await asyncio.sleep(retry_delay)  # Attendre avant de réessayer de créer le filtre
            ...
            try:
                ...
                while True:  # Boucle de lecture des événements
                    try:
                        # Railway-specific: Check connection health periodically
                        if not self.w3_ws.is_connected():
                            logger.warning("WebSocket connection lost during V4 event loop, breaking to reconnect...")
                            break
    ...
    async def listen_onchain_doppler(self):
        await self.bot.wait_until_ready()
        if not self.channel:
            self.channel = self.bot.get_channel(CHANNEL_ID)
        channel = self.channel
        if not channel:
            logger.error("Could not find channel for Doppler notifications")
            return
        ...
        while True:  # Boucle principale de reconnexion
            try:
                logger.info("Started on-chain Doppler event listener")
                ...
                while True:  # Boucle de lecture des événements
                    try:
                        ...
                        # Look for recent transactions to Doppler factory (optimized for Railway)
                        doppler_txs = []
                        # Railway-specific: Reduce block range to avoid timeouts
                        for block_num in range(latest_block - 5, latest_block + 1):  # Check last 5 blocks only
                            try:
                                block = self.w3_ws.eth.get_block(block_num, full_transactions=True)
                                for tx in block.transactions:
                                    if tx['to'] and tx['to'].lower() == DOPPLER_FACTORY_ADDRESS.lower():
                                        # Check if this is a create function call
                                        if tx['input'].startswith('0x') and len(tx['input']) > 10:  # Has input data
                                            doppler_txs.append(tx)
                            except Exception as e:
                        ...
                        for tx in doppler_txs:
                            try:
                                # Decode the create function call
                                func_obj, func_args = self.doppler_factory.decode_function_input(tx['input'])
                                ...
                                if recipient.lower() in self.seen_doppler_tokens:
                                    continue
                                
                                # Add to seen tokens
                                self.seen_doppler_tokens.add(recipient.lower())
                                self._save_seen_doppler_tokens()
                                ...
                                # Envoie l'alerte Discord
                                embed = discord.Embed(
                                    title="🆕 Nouveau Token Doppler (on-chain)",
                                    color=discord.Color.blue(),
                                    timestamp=datetime.now(timezone.utc)
                                )
                                embed.add_field(name="Nom du Token", value=name, inline=True)
                                embed.add_field(name="Ticker", value=symbol, inline=True)
                                embed.add_field(name="Adresse", value=f"`{recipient}`", inline=False)
                                embed.add_field(name="Owner", value=f"`{owner}`", inline=False)
                                embed.add_field(name="Supply Initial", value=f"{initial_supply:,}", inline=True)
                                
                                # Add transaction link
                                tx_link = f"https://basescan.org/tx/{tx['hash'].hex()}"
                                embed.add_field(name="Transaction", value=f"[Voir sur Basescan]({tx_link})", inline=False)
                                ...
                                await channel.send(embed=embed, view=view)
                                logger.info(f"On-chain Doppler alert sent for {name} ({symbol}) {recipient}")
                                
                                # Add to volume tracking
                                self.tracked_doppler_tokens[recipient.lower()] = {
                                    'first_seen': time.time(),
                                    'alerted': False
                                }
                                logger.info(f"[VOLUME TRACK] Ajout du token Doppler {recipient.lower()} à la surveillance volume (on-chain)")
                                
                            except Exception as e:
                                logger.error(f"Error processing Doppler transaction: {e}")
                                continue
                        ...
                        logger.error(f"Error in on-chain Doppler event loop: {e}")
                        await asyncio.sleep(10)  # Longer delay on error
                        
            except Exception as e:
                logger.error(f"Error in Doppler monitoring: {e}")
...
class Bot(commands.Bot):
    ...
    async def setup_hook(self):
        ...
        asyncio.create_task(clanker_monitor.listen_onchain_doppler())
